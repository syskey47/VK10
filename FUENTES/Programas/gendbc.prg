...
IF PARAMETERS() < 1 OR TYPE("cOutFile") <> "C"
	m.cOutFile = ""
...
*! as procedures
m.cFile = UPPER(SUBSTR(m.cOutFile, 1, AT(".", m.cOutFile))) + "krt"
...
IF m.nTotal_Relations > 0
	...
	FOR m.nLoop = 1 TO m.nTotal_Relations
		REPLACE Program WITH "ALTER TABLE '" + aAll_Relations(m.nLoop, 1) +;
							 "' ADD FOREIGN KEY TAG " +;
